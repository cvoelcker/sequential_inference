...
from envs.sawyer.mujoco.mujoco_env import MujocoEnv, _assert_task_is_set
...
class SawyerMocapBase(MujocoEnv):
    ...
    def get_env_state(self):
        ...
        return dict(joint=joint_state, mocap=mocap_state)
        # return copy.deepcopy(state)

    def set_env_state(self, state):
        joint_state = state["joint"]
        mocap_state = state["mocap"]
...
class SawyerXYZEnv(SawyerMocapBase):
    _HAND_SPACE = Box(
        np.array([-0.525, 0.35, -0.0525]), np.array([+0.525, 1.025, 0.525])
    )
    max_path_length = 200
    ...
    def set_xyz_action_rot(self, action):
        ...
        quat = (
            ...
            * Quaternion(axis=list(rot_axis), angle=action[3])
        ).elements
    ...
    def set_xyz_action_rotz(self, action):
        ...
        new_mocap_zangle = self.quat_to_zangle(self.data.mocap_quat[0]) + zangle_delta
        ...
        self.data.set_mocap_quat("mocap", self.zangle_to_quat(new_mocap_zangle))

    def quat_to_zangle(self, quat):
        angle = (
            Quaternion(axis=[0, 1, 0], angle=(np.pi / 2)).inverse * Quaternion(quat)
        ).angle
        return angle
    ...
    def zangle_to_quat(self, zangle):
        ...
        return (
            ...
            * Quaternion(axis=[-1, 0, 0], angle=zangle)
        ).elements
    ...
    def _reset_hand(self, steps=50):
        for _ in range(steps):
            ...
            self.data.set_mocap_quat("mocap", np.array([1, 0, 1, 0]))
            # here we reset the gripper to be closed
