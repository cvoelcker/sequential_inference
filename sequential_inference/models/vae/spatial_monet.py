...
class SpatialLocalizationNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        constrain_add = torch.tensor([[[self.min, 0.0, -1], [0.0, self.min, -1]]])
        self.register_buffer("constrain_add", constrain_add)
        constrain_mult = torch.tensor(
            [[[self.max - self.min, 0.0, 2.0], [0.0, self.max - self.min, 2.0]]]
        )
        self.register_buffer("constrain_mult", constrain_mult)
        constrain_scale = torch.tensor([[[0.99, 0.0, 2.0], [0.0, 0.99, 2.0]]])
        self.register_buffer("constrain_scale", constrain_scale)
        constrain_shift = torch.tensor([[[0.01, 0.0, -1.0], [0.0, 0.01, -1.0]]])
        self.register_buffer("constrain_shift", constrain_shift)
        constrain_std = torch.tensor([[[1.0, 0.0, 1.0], [0.0, 1.0, 1.0]]])
    ...
    def forward(self, x):
        ...
        theta_std = (0.1 * theta[:, :, 1] * self.constrain_std) + 1e-4
...
class MaskNet(nn.Module):
    ...
    def __init__(self, in_channels=7, num_blocks=None, channel_base=32, **kwargs):
        super().__init__()
        self.unet = net_util.UNet(
...
class SpatialAutoEncoder(nn.Module):
    ...
    def forward(self, x, theta):
        ...
        p_x = net_util.reconstruction_likelihood(
            x[:, :3], x_reconstruction, mask, self.fg_sigma
        )
    ...
    def encode(self, x, theta):
        ...
        z_img, kl_z_img = net_util.differentiable_sampling(
            mean_img, sigma_img, self.prior
        )

        mean_mask, sigma_mask = self.mask_encoding_network(encoder_input)
        sigma_mask = F.softplus(sigma_mask) + 1e-10
        z_mask, kl_z_mask = net_util.differentiable_sampling(
            mean_mask, sigma_mask, self.prior
        )
    ...
    def decode(self, z_img, z_mask, theta):
        ...
        x_reconstruction = net_util.invert(
            patch_reconstruction, theta, self.image_shape
        )
...
class MaskedAIR(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.spatial_vae = SpatialAutoEncoder(
            latent_prior,
            latent_dim,
            fg_sigma,
            bg_sigma,
        ...
        self.spatial_localization_net = SpatialLocalizationNet(
            image_shape, patch_shape, constrain_theta, num_slots
        )
    ...
    def build_image_graph(self, x):
        ...
        grid_interactions = embeddings_interaction - embeddings_interaction.permute(
            0, 2, 1, 3
        )
    ...
    def forward(self, x):
        ...
        loss = torch.zeros_like(x[:, 0, 0, 0])
        kl_zs = torch.zeros_like(x[:, 0, : self.latent_dim, 0]).squeeze(-1)
        kl_masks = torch.zeros_like(x[:, 0, :, :]).view(
        ...
        for i in range(self.num_slots):
            ...
            thetas.append(theta)
            inp = torch.cat(
                [x, ((x - background) - total_reconstruction).detach(), scope], 1
            )
            (
                x_recon,
                mask,
        ...
        # calculate reconstruction error
        p_x = net_util.reconstruction_likelihood(
            x, background, (1 - torch.sum(torch.cat(masks, 1), 1, True)), self.bg_sigma
        )
        p_x_loss += p_x
    ...
    def build_flat_image_representation(self, x, return_dists=False):
        ...
        grid = (
            net_util.center_of_mass(masks[:, 1:] + 1e-20) - (self.image_shape[0] / 2)
        ) / self.image_shape[0]
        ...
        if return_dists:
            ...
            full = torch.cat([grid, pos.view(-1, self.num_slots, 6), latents_mean], -1)
            return full, loss
    ...
    def reconstruct_from_latent(self, x, imgs=None, reconstruct_mask=True):
        ...
        images = torch.zeros(
            x.shape[0], 3, self.image_shape[0], self.image_shape[1]
        ).cuda()
        if imgs is not None:
        ...
        for i in range(self.num_slots):
            ...
            if not reconstruct_mask:
                mask_pred = mask
                inp = torch.cat(
                    [imgs, ((imgs - background) - images).detach(), scope], 1
                )
                grid = F.affine_grid(
                    thetas[:, i], torch.Size((imgs.size()[0], 1, *self.patch_shape))
                )
            ...
            if imgs is not None:
                p_x += net_util.reconstruction_likelihood(
                    imgs, recon, mask * scope, self.fg_sigma, self.running
                )
