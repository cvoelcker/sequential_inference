...
class Dynamics(nn.Module):
    ...
    def __init__(self, config, monet_config, stove_config, seed=None):
        ...
        self.dyn_inference = nn.Sequential(
            nn.Linear(cl, cl), nn.ReLU(), nn.Linear(cl, self.num_dynamics)
        )
        ...
        if z_std is not None:
            ...
            z_std = torch.cat(
                [
                    ...
                    self.c.debug_latent_q_std * torch.sigmoid(z_std[..., 8:]),
                ],
                -1,
            )
    ...
    def forward(self, s, actions=None):
        ...
        for core_idx in range(self.num_dynamics):
            ...
            result += (self.out[core_idx][1](out1) + out1) * dyn_types[
                ..., core_idx : core_idx + 1
            ]
